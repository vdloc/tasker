...
export default function TagsDropDown({
  ...
  const selectableTags = tags.filter(({ id }) =>
    excludeTags.every((excludeTag) => (excludeTag as Tag).id !== id)
  );
  ...
  return (
    <Menu as='div' className='relative inline-block text-left'>
      ...
      >
        <Menu.Items className='absolute bottom-0 md:bottom-auto left-0 md:left-auto md:right-0 z-10 mt-2 w-40 h-52 overflow-y-scroll origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none'>
          <div className='py-1'>
            {selectableTags.map((tag) => (
              <TagDropDownMenuItem
                tag={tag}
                key={tag.id}
                handleClick={() => onChange(tag)}
              />
            ))}
          </div>
        </Menu.Items>
