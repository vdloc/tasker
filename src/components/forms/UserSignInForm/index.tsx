...
import firebaseMessages from '@data/firebase-messages.json';
...
export default function UserSignInForm() {
  ...
  const { toggleNotification, setNotification } = useNotificationStore(
    (state) => state,
    shallow
  );
  ...
  async function onSubmit(data: UserSignInFormValues) {
    ...
    try {
      ...
      if (firebaseError.code in firebaseMessages.errors) {
        const errorCode = firebaseError.code as string;
        const firebaseErrorMessages =
          firebaseMessages.errors as FirebaseErrorMessages;
        setNotification({
          icon: () => <FaceFrownIcon className='w-6 h-6 text-red-600' />,
          title: 'Authentication Error',
          description: firebaseErrorMessages[errorCode],
        });
  ...
  return (
    <div className='w-full h-screen md:h-auto flex flex-col justify-center overflow-hidden md:w-[20rem] py-6 px-4 mx-auto relative z-10 rounded-2xl bg-white shadow-2xl drop-shadow-2xl'>
      ...
      <form
        className='py-4 divide-y-2 divide-dashed'
